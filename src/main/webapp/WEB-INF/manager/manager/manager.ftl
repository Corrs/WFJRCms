...
<@ms.content>
		...
		<#include "${managerViewPath}/include/popover.ftl"/>  
		<!--提示模态框开始-->
		<#-- <@ms.modal modalName="alertSuccess" title="提示">
			 <@ms.modalBody>
		     </@ms.modalBody>
		     <@ms.modalButton>
	    		<@ms.button value="关闭" id="closeAlertSuccess"/>  
	 		 </@ms.modalButton>
		</@ms.modal> -->
		<!--提示模态框结束-->
			...
			if(vobj.isValid()){
					$.ajax({
					   ...
					   success: function(msg){ 
					   		if(msg.result==true){
					   			//alert(buttonHtml+"成功");
					   			showPopoverModal(buttonHtml+"成功", function(){
					   				location.reload();
					   			});
					   			//location.reload();
					   		}else{
					   			showPopoverModal(msg.resultMsg);
					   			$("#updateButton").text(buttonHtml);
					   			$("#updateButton").attr("disabled",false);
					   		}
					   },error: function(){
					   	  showPopoverModal("数据请求失败，请检查请求地址和参数是否正确");
					   	  //showPopoverModal("数据请求失败，请检查请求地址和参数是否正确");
	...
	$(".updateModalBtn").click(function(){
		...
		URL="${basePath}/${baseManager}/manager/"+managerId+"/edit.do";
		$.ajax({
			type: "post",
			dataType:"json",
			url:  URL,
			...
			if(checkboxData.length != 0) {
				$.ajax({
				   ...
				   success: function(msg){
				   	  ...
				   	  showPopoverModal("批量删除管理员失败");
				      $(".deleteAll").modal("hide");
				   }
				});
			 } else {
				showPopoverModal("删除失败，请先选择管理员");
				$(".deleteAll").modal("hide");
			 }
		 });
